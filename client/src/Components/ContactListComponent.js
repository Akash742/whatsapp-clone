import React, { setChat, useState, useEffect } from "react";
...
const ContactComponent = (props) => {
  ...
  const lastMessage = userData.messages && userData.messages.length? userData.messages[userData.messages.length - 1] : {};
  ...
  return (
    <ContactItem onClick={() => setChat({ channelData: userData, otherUser })}>
      ...
      <MessageTime>{lastMessage && new Date(lastMessage?.addedOn).getUTCDate()}</MessageTime>
    </ContactItem>
  );
};
const ContactListComponent = (props) => {
  ...
  return (
    <Container>
      <ProfileInfoDiv>
        <ProfileImage src={userInfo.picture || "/profile/akash.jpg"} />
      </ProfileInfoDiv>
      <SearchBox>
        <SearchContainer>
